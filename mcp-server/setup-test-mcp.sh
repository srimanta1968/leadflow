    ...
    if [ -n "$ws" ]; then
        ...
        if [ -d "$ws/mcp-server" ] && [ -f "$ws/mcp-server/$(basename "$COMPOSE_FILE")" ]; then
            ...
            if [ -n "$a" ] && [ "$a" != "$b" ]; then
                ...
                # the caller is `cd "$(resolve_compose_dir)"`, so anything echoed
                # here becomes part of the path. These two lines were going to
                # stdout, which made the argument the ANSI-coloured banner text
                # WITH the directory appended — and `cd` then failed with the
                # whole escape sequence quoted back. The container could not be
                # restarted at all once it stopped, which is exactly when this
                # path runs.
                print_msg "$YELLOW" "[INFO] Composing from the OWNER project so other projects keep their mounts:" >&2
                print_msg "$YELLOW" "       $a" >&2
                echo "$a"; return
            fi
        fi
    fi
